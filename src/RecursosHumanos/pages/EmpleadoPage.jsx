...
export const EmpleadoPage = () => {
  ...
  const condicional = 216;
 
   // Filtrar los datos con estado "activo"
   const datosActivos = datos.filter(empleado => empleado.status === 'activo');
   const datosInactivos = datos.filter(empleado => empleado.status === 'baja');


   // Obtener la longitud de los datos activos
   const cantidadEmpleadosActivos = datosActivos.length;
   const cantidadEmpleadosInactivos = datosInactivos.length;
  console.log(cantidadEmpleadosActivos)
    ...
    <Container maxWidth="lg">
      <Paper ref={paperRef} sx={{ mt: 5, marginBottom: '60px', boxShadow: 10, padding: 2, backgroundColor: "#F5F5F5" }}>
        <EmpleadoButtons
          ...
          isAddDisabled={cantidadEmpleadosActivos >= condicional}
        ...
        {selectedRows.length > 0 && (
          <Box mt={2} textAlign="center">
            ...
            <Typography variant="subtitle1">
              Empleados Activos: {cantidadEmpleadosActivos} y Empleados inactivos: {cantidadEmpleadosInactivos}
            </Typography>
          </Box>
        )}
        <EmpleadoTable
